...
def test_research_ask_request_rejects_whitespace_only_question_without_hitl():
    with pytest.raises(ValidationError, match="question不能为空"):
        ResearchAskRequest(**_base_payload(question="\n\t  "))
...
def test_research_ask_request_allows_empty_question_for_hitl_feedback_values(
    feedback,
):
    req = ResearchAskRequest(
...
def test_research_ask_request_empty_interrupt_feedback_still_requires_question():
    with pytest.raises(ValidationError, match="question不能为空"):
        ResearchAskRequest(**_base_payload(question="", interrupt_feedback=""))
