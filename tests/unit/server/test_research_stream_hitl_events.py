...
@pytest.mark.asyncio
async def test_process_langgraph_stream_tool_result_includes_finish_reason():
    """Tool result events must forward finish_reason when present on ToolMessage."""
    message = ToolMessage(
...
@pytest.mark.asyncio
async def test_process_langgraph_stream_tool_calls_includes_reasoning_metadata():
    """Tool call events must forward reasoning_content/finish_reason like message chunks."""
    message = AIMessageChunk(
        content="",
        id="msg-tool-reason",
        tool_calls=[{"name": "web_search", "args": {"query": "AI"}, "id": "call-1"}],
        additional_kwargs={"reasoning_content": "planning search"},
        response_metadata={"finish_reason": "tool_calls"},
    )
    ...
    tool_calls = next(event for event in events if event["event"] == "tool_calls")
    payload = json.loads(tool_calls["data"])
    assert payload["reasoning_content"] == "planning search"
    assert payload["finish_reason"] == "tool_calls"


@pytest.mark.asyncio
async def test_process_langgraph_stream_skips_complete_when_aget_state_fails():
    """Stream must not crash when LangGraph state lookup fails after successful execution."""
    ...
    mock_graph.aget_state = AsyncMock(side_effect=RuntimeError("state unavailable"))
...
@pytest.mark.asyncio
async def test_process_langgraph_stream_tool_call_chunks_includes_reasoning_metadata():
    """Streaming tool_call_chunks must forward reasoning_content/finish_reason like tool_calls."""
    message = AIMessageChunk.model_construct(
        content="",
        id="msg-chunk-reason",
        tool_calls=[],
        ...
        additional_kwargs={"reasoning_content": "streaming tool args"},
        response_metadata={"finish_reason": "tool_calls"},
    )
    ...
    chunks = next(event for event in events if event["event"] == "tool_call_chunks")
    payload = json.loads(chunks["data"])
    assert payload["reasoning_content"] == "streaming tool args"
    assert payload["finish_reason"] == "tool_calls"


@pytest.mark.asyncio
async def test_process_langgraph_stream_uses_nested_research_config_when_present():
    """Nested research_config from ask API must bypass flattened fallback parsing."""
    session_repo = MagicMock()
    session_repo.get_session_by_thread_id = AsyncMock(
        return_value=MagicMock(id=1, thread_id="thread-1")
    )
    ...
    assert any(event["event"] == "metadata" for event in events)
    configurable = captured["config"]["configurable"]
    assert configurable["max_plan_iterations"] == 10
    assert configurable["max_step_num"] == 12
    assert configurable["max_search_results"] == 15
    assert configurable["report_style"] == "casual"
    assert configurable["enable_deep_thinking"] is True
