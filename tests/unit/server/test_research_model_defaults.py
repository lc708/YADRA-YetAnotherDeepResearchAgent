...
def _resolve_create_execution_model(model_config: dict) -> tuple[str, str]:
...
def test_create_api_execution_model_defaults_match_haiku_4_5():
    """Regression: create/followup execution records must default to claude-haiku-4-5."""
    model_used, provider = _resolve_create_execution_model({})
    assert model_used == "claude-haiku-4-5"
    assert provider == "anthropic"


def test_model_defaults_respect_explicit_overrides():
    custom = {"model_name": "custom-model", "provider": "openai"}
    stream_info = _resolve_stream_model_info({"model_config": custom})
    create_model, create_provider = _resolve_create_execution_model(custom)
    assert stream_info["model_name"] == "custom-model"
    assert stream_info["provider"] == "openai"
    assert create_model == "custom-model"
