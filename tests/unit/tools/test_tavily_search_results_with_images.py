...
def _tool_with_mock_api():
    """Build tool without Tavily API key validation (Pydantic v2 field validators)."""
    return TavilySearchResultsWithImages.model_construct(
...
def test_tavily_search_run_accepts_langchain_core_callback_manager():
    """PR #28: callback imports moved to langchain_core; sync path must still accept managers."""
    from langchain_core.callbacks import CallbackManagerForToolRun

    tool = _tool_with_mock_api()
    tool.api_wrapper.raw_results.return_value = {"results": [], "images": []}
    tool.api_wrapper.clean_results_with_images.return_value = []

    result, raw = tool._run("test query", run_manager=CallbackManagerForToolRun.get_noop_manager())

    assert result == []
    assert raw == {"results": [], "images": []}


@pytest.mark.asyncio
async def test_tavily_search_arun_accepts_langchain_core_async_callback_manager():
    """PR #28: async callback imports moved to langchain_core."""
    from langchain_core.callbacks import AsyncCallbackManagerForToolRun

    tool = _tool_with_mock_api()
    tool.api_wrapper.raw_results_async = AsyncMock(return_value={"results": [], "images": []})
    tool.api_wrapper.clean_results_with_images.return_value = []

    result, raw = await tool._arun(
        "test query",
        run_manager=AsyncCallbackManagerForToolRun.get_noop_manager(),
    )
