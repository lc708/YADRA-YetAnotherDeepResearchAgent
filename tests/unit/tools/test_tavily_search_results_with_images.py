...
class TestTavilySearchResultsWithImages:
    ...
    def mock_api_wrapper(self):
        """Create a mocked API wrapper."""
        return Mock(spec=EnhancedTavilySearchAPIWrapper)

    @pytest.fixture
    def search_tool(self, mock_api_wrapper):
        """Create TavilySearchResultsWithImages instance with mocked wrapper."""
    ...
    def test_return_value_format(
        ...
        """Test that return values have the correct format."""
        mock_api_wrapper.raw_results.return_value = sample_raw_results
        mock_api_wrapper.clean_results_with_images.return_value = sample_cleaned_results
    ...
    @pytest.mark.asyncio
    async def test_async_return_value_format(
        self,
        search_tool,
        mock_api_wrapper,
        sample_raw_results,
        ...
        """Test that async return values have the correct format."""
        mock_api_wrapper.raw_results_async = AsyncMock(return_value=sample_raw_results)
        mock_api_wrapper.clean_results_with_images.return_value = sample_cleaned_results
