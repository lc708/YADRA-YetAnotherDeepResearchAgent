...
def patch_chat_clients(monkeypatch):
...
def test_create_llm_http_client_uses_secure_timeouts_and_limits(
    ...
    assert captured["limits"].max_connections == 20
    assert captured["limits"].max_keepalive_connections == 10
    assert result.kwargs["max_retries"] == 3
    assert result.kwargs["timeout"] == 120.0
    assert result.kwargs["request_timeout"] == 120.0


def test_create_llm_reasoning_maps_base_url_to_api_base(dummy_conf):
    reasoning_conf = {
        **dummy_conf,
        "REASONING_MODEL": {
            "api_key": "reason_key",
