    ...
    def print_latest_data(self, data: Dict[str, Any]):
        ...
        if messages:
            ...
            for msg in messages[:2]:  # 只显示前2条
                print(
                    f"   - [{msg['role']}] {msg['content_type']} | {msg['source_agent']} | {msg['content_length']}字符"
                )
        ...
        if artifacts:
            ...
            for art in artifacts[:2]:  # 只显示前2个
                print(
                    f"   - {art['type']} | {art['title']} | {art['source_agent']} | {art['content_length']}字符"
                )
        ...
        if executions:
            ...
            print(
                f"⚡ 执行状态: {latest_exec['status']} | {latest_exec['action_type']}"
            )
    ...
    async def get_final_analysis(self) -> Dict[str, Any]:
        ...
        if artifacts:
            ...
            analysis["artifact_analysis"] = {
                ...
                "has_research_plan": any(
                    art["type"] == "research_plan" for art in artifacts
                ),
                            ...
                            try:
                                ...
                                if current_event_type in [
                                    ...
                                    if current_event_type == "metadata":
                                        print(
                                            f"   - thread_id: {event_data.get('thread_id', 'N/A')}"
                                        )
                                        print(
                                            f"   - execution_id: {event_data.get('execution_id', 'N/A')}"
                                        )
                                    elif current_event_type == "artifact":
                                        print(
                                            f"   - artifact_type: {event_data.get('type', 'N/A')}"
                                        )
                                        print(
                                            f"   - artifact_title: {event_data.get('title', 'N/A')}"
                                        )

                                event_count += 1
                                if (
                                    event_count >= 8
                                ):  # 收到更多事件后停止，确保能看到artifact
                                    break
...
async def main():
    ...
    if artifact_analysis:
        ...
        print(
            f"  - 有研究计划: {'✅' if artifact_analysis['has_research_plan'] else '❌'}"
        )
