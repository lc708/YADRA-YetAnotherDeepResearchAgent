...
async def test_auto_accept_plan_fix():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post(
            f"{base_url}/stream", json=test_config_false
        ) as response:
            if response.status == 200:
                    ...
                    if line_str.startswith("data: "):
                        try:
                            ...
                            if event_count <= 5:  # 只显示前5个事件
                                print(
                                    f"   事件 {event_count}: {data.get('event', 'unknown')}"
                                )
                            ...
                            # 检查是否直接生成了最终报告
                            if data.get("event") == "artifact" and "summary" in str(
                                data
                            ):
                ...
                if plan_generated and not final_report_generated:
                    ...
                    print(
                        f"   ⚠️  测试不确定：plan_generated={plan_generated}, final_report_generated={final_report_generated}"
                    )
            else:
        ...
        async with session.post(
            f"{base_url}/stream", json=test_config_true
        ) as response:
            if response.status == 200:
                plan_generated = False
                final_report_generated = False
