    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            async with session.get(url) as response:
                ...
                print(
                    f"📡 GET /api/research/workspace/{test_url_param} - Status: {response.status}"
                )
                ...
                if response.status == 200:
                    ...
                    print(
                        f"   - Session metadata: {'Yes' if data.get('sessionMetadata') else 'No'}"
                    )
                else:
        ...
        if request_count["workspace_get"] == 1:
            ...
            print(
                f"❌ ISSUE: {request_count['workspace_get']} workspace requests detected"
            )
