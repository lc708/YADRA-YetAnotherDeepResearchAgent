                            ...
                            try:
                                if event_content:
                                    ...
                                    if event_type == "navigation":
                                        print(
                                            f"   🔗 Workspace URL: {parsed_data.get('workspace_url', 'N/A')}"
                                        )
                                        print(
                                            f"   🆔 URL Param: {parsed_data.get('url_param', 'N/A')}"
                                        )
                                        print(
                                            f"   🧵 Thread ID: {parsed_data.get('thread_id', 'N/A')}"
                                        )
                                    ...
                                    # 特殊处理错误事件
                                        ...
                                        print(
                                            f"   ❌ Error: {parsed_data.get('error_message', 'Unknown error')}"
                                        )
                                        break
...
async def test_ask_traditional():
    ...
    request_data = {
        ...
        "visitor_id": str(uuid.uuid4()),
        "user_id": None,
        "config": {"auto_accepted_plan": False, "reportStyle": "academic"},
    }
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(
                f"{base_url}/api/research/ask?stream=false", json=request_data
            )
