            ...
            async with session.post(url, json=test_payload) as response:
                    ...
                    if line.startswith('event: '):
                        ...
                        try:
                            ...
                            if current_event_type == 'navigation':
                                ...
                                print(f"   execution_id: {event_data.get('execution_id')}")
                                
                            # 收到足够事件后停止
                            if len(navigation_events) >= 1 and len(metadata_events) >= 1:
                                print("\n🛑 收到足够事件，停止测试")
                                break
                ...
                if len(navigation_events) == 1:
                    ...
                    if nav_event.get('session_id'):
                        print(f"✅ navigation事件包含session_id: {nav_event['session_id']}")
                    else:
                    ...
                    print(f"❌ 收到{len(navigation_events)}个navigation事件（仍有重复）")
                    return False
                ...
                return len(navigation_events) == 1 and navigation_events[0].get('session_id')
