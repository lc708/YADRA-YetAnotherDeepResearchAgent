    ...
    try:
        ...
        async with await repo.get_connection() as conn:
            ...
            for s in sessions:
                ...
                print(f'    → Executions: {exec_data[0]}, Last: {exec_data[1]}, Status: {exec_data[2]}')
                ...
                if exec_data[0] > 0:
                    ...
                    for exec in exec_details:
                        print(f'      • Exec: {exec["execution_id"][:20]}..., Action: {exec["action_type"]}, Status: {exec["status"]}, Created: {exec["created_at"]}')
