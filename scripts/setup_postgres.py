...
def get_connection_strings(project_ref, password):
    """构建多种PostgreSQL连接字符串"""
    connections = {
        "direct": f"postgresql://postgres:{password}@db.{project_ref}.supabase.co:5432/postgres",
        "session_pooler": f"postgresql://postgres.{project_ref}:{password}@aws-0-ap-southeast-1.pooler.supabase.com:5432/postgres",
        "transaction_pooler": f"postgresql://postgres.{project_ref}:{password}@aws-0-ap-southeast-1.pooler.supabase.com:6543/postgres"
    }
    return connections
...
def main():
    ...
    if not db_password:
        ...
        print(f"DATABASE_URL=postgresql://postgres:${{SUPABASE_DB_PASSWORD}}@db.{project_ref}.supabase.co:5432/postgres")
        print("\n获取密码的步骤:")
    ...
    if test_database_connection(connections["session_pooler"], "Session Pooler"):
        ...
        print(f"DATABASE_URL=postgresql://postgres.{project_ref}:${{SUPABASE_DB_PASSWORD}}@aws-0-ap-southeast-1.pooler.supabase.com:5432/postgres")
        print("ℹ️  Session Pooler: IPv4支持，适合持久服务器应用，支持prepared statements")
        return True
    
    # 2. 尝试Transaction Pooler (推荐用于无服务器函数，仅IPv4)
    print("\n2️⃣ 尝试Transaction Pooler (IPv4支持，适合无状态应用)...")
    if test_database_connection(connections["transaction_pooler"], "Transaction Pooler"):
        print(f"\n✅ Transaction Pooler可用!")
        print(f"📄 请更新你的 .env 文件:")
        print(f"DATABASE_URL=postgresql://postgres.{project_ref}:${{SUPABASE_DB_PASSWORD}}@aws-0-ap-southeast-1.pooler.supabase.com:6543/postgres")
        print("ℹ️  Transaction Pooler: IPv4支持，适合无状态/serverless应用")
        print("⚠️  注意: Transaction Pooler不支持prepared statements，可能影响LangGraph性能")
        return True
