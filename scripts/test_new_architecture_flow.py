                        ...
                        if line.startswith("event:"):
                            ...
                            try:
                                ...
                                if event_type == "navigation":
                                    ...
                                    print(
                                        f"      URL参数: {navigation_data.get('url_param')}"
                                    )
                                    print(
                                        f"      Thread ID: {navigation_data.get('thread_id')}"
                                    )
                                    print(
                                        f"      工作区URL: {navigation_data.get('workspace_url')}"
                                    )
                                    # 不要break，继续接收后续事件

                                elif event_type == "metadata":
                                    print(
                                        f"      执行ID: {event_data.get('execution_id')}"
                                    )
                                    print(
                                        f"      预计时长: {event_data.get('estimated_duration')}秒"
                                    )

                                elif event_type == "progress":
                                    print(
                                        f"      当前步骤: {event_data.get('current_step')}"
                                    )
                                    print(
                                        f"      进度: {event_data.get('progress_percentage')}%"
                                    )
                                    print(
                                        f"      状态: {event_data.get('status_message')}"
                                    )

                                elif event_type == "message_chunk":
                                    print(
                                        f"      消息块: {event_data.get('content')[:100]}..."
                                    )

                                elif event_type == "artifact":
                                    print(
                                        f"      Artifact类型: {event_data.get('type')}"
                                    )
                                    print(f"      标题: {event_data.get('title')}")

                                elif event_type == "complete":
                                    print(
                                        f"      执行完成，耗时: {event_data.get('total_duration')}ms"
                                    )
                                    print(
                                        f"      Token消耗: {event_data.get('tokens_consumed')}"
                                    )
                                    print("   🎉 工作流执行完成！")
                                    # 在complete事件后可以结束
                                    break
                                    ...
                                    print(
                                        f"      错误代码: {event_data.get('error_code')}"
                                    )
                                    print(
                                        f"      错误信息: {event_data.get('error_message')}"
                                    )
                                    print("   ❌ 工作流执行出错！")
                                    break
                        ...
                        if workspace_url_param:
                            async with session.get(
                                ...
                                if workspace_response.status == 200:
                                    ...
                                    print("   ✅ 工作区状态获取成功")
                                    print(
                                        f"      Thread ID: {workspace_data.get('threadId')}"
                                    )
                                    print(f"      状态: {workspace_data.get('status')}")
                                    print(
                                        f"      消息数量: {len(workspace_data.get('messages', []))}"
                                    )
                                    print(
                                        f"      Artifacts数量: {len(workspace_data.get('artifacts', []))}"
                                    )
                                    ...
                                    if db_url:
                                        ...
                                        # 通过URL参数查询会话
                                        session_mapping = (
                                            await repo.get_session_by_url_param(
                                                workspace_url_param
                                            )
                                        )
                                        if session_mapping:
                                            ...
                                            print(
                                                f"      Thread ID: {session_mapping.thread_id}"
                                            )
                                            print(
                                                f"      URL参数: {session_mapping.url_param}"
                                            )
                                            print(
                                                f"      状态: {session_mapping.status}"
                                            )
                                            print(
                                                f"      创建时间: {session_mapping.created_at}"
                                            )
                                        else:
                                            print("   ❌ 数据库中未找到会话映射")
                                    else:
