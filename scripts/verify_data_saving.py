    ...
    try:
        async with await psycopg.AsyncConnection.connect(
            ...
            for session in sessions:
                print(
                    f"  - {session['thread_id'][:8]}... | {session['url_param'][:20]}... | {session['status']} | {session['created_at']}"
                )
            ...
            # 获取session_id
            await cursor.execute(
                "SELECT id FROM session_mapping WHERE thread_id = %s", (thread_id,)
            )
            ...
            if message_stats["count"] > 0:
                ...
                for msg in recent_messages:
                    print(
                        f"    - {msg['role']} | {msg['content_type']} | {msg['source_agent']} | {msg['content_preview'][:50]}..."
                    )
            ...
            if artifact_stats["count"] > 0:
                ...
                for artifact in recent_artifacts:
                    print(
                        f"    - {artifact['type']} | {artifact['title']} | {artifact['source_agent']} | {artifact['content_length']}字符"
                    )
            ...
            print(
                f"  - LangGraph数据 (checkpoints): {'✅' if has_checkpoints else '❌'}"
            )
            print(f"  - 消息数据 (message_history): {'✅' if has_messages else '❌'}")
            print(
                f"  - Artifact数据 (artifact_storage): {'✅' if has_artifacts else '❌'}"
            )
