...
async def test_chat_stream_integration():
    ...
    test_message = {
        "messages": [
            {
                "role": "user", 
                "content": "Hello, can you test reasoning mode with a simple math question: what is 15 + 27?"
            }
        ],
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                    ...
                    while '\n\n' in buffer:
                        ...
                        if event_type == 'message_chunk' and data_content:
                            try:
                                ...
                                print(f"📨 Message {message_count} from {agent}: {content[:50]}{'...' if len(content) > 50 else ''}")
                                
                                # 检查推理内容
                                if 'reasoning_content' in data:
                                    reasoning_found = True
                                    reasoning_content = data['reasoning_content']
                                    print(f"🧠 Reasoning content found: {reasoning_content[:100]}...")
                ...
                print(f"🧠 Reasoning content found: {'✅' if reasoning_found else '❌'}")
