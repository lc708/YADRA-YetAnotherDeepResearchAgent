...
async def test_create_and_check_data():
    ...
    create_data = {
        ...
        "config": {
            ...
            "output_config": {"output_format": "markdown"},
        },
    }
                    ...
                    if artifacts:
                        ...
                        for i, art in enumerate(artifacts):
                            ...
                            content_preview = (
                                ...
                                else art.get("content", "")
                            )
                            print(
                                f"     {i+1}. [{art_type}] {title}: {content_preview}"
                            )
                ...
                try:
                    ...
                    if state_snapshot and state_snapshot.values:
                        ...
                        if "messages" in state_snapshot.values:
                            ...
                            print(
                                f"   Messages count: {len(messages) if isinstance(messages, list) else 'N/A'}"
                            )
...
async def test_checkpoint_creation():
    ...
    try:
        ...
        if state_snapshot and state_snapshot.values:
            ...
            print(f"   State keys: {list(state_snapshot.values.keys())}")
            print(
                f"   Research topic: {state_snapshot.values.get('research_topic', 'N/A')}"
            )
        else:
