            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if login_response.status_code == 200:
                        ...
                        print(f"✅ 登录成功！")
                        print(
                            f"   Token: {login_result['session']['access_token'][:30]}..."
                        )

                        # 测试获取用户信息
                        headers = {
                            "Authorization": (
                                f"Bearer {login_result['session']['access_token']}"
                            )
                        }
                        me_response = await client.get(
                            f"{BASE_URL}/auth/me", headers=headers
                        )
    ...
    async with httpx.AsyncClient() as client:
        ...
        if login_response.status_code == 200:
            ...
            chat_response = await client.post(
                f"{BASE_URL}/chat/stream", json=chat_data, headers=headers, timeout=5.0
            )
