    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                async for line in response.content:
                    ...
                    if line.startswith("data: "):
                        try:
                            ...
                            print(
                                f"   数据: {json.dumps(data, indent=2, ensure_ascii=False)}"
                            )
                            ...
                            # 保存interrupt数据
                                ...
                                print(
                                    f"   Message: {data.get('content', data.get('message'))}"
                                )
                ...
                print(
                    f"Feedback请求数据: {json.dumps(feedback_data, indent=2, ensure_ascii=False)}"
                )
                    ...
                    async for line in feedback_response.content:
                        ...
                        if line.startswith("data: "):
                            try:
                                ...
                                print(
                                    f"📨 Feedback事件 {feedback_event_count}: {event_type}"
                                )
                                print(
                                    f"   数据: {json.dumps(data, indent=2, ensure_ascii=False)}"
                                )
