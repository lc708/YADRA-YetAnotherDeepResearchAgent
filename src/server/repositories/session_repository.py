...
class SessionRepository:
    ...
    async def get_connection(self) -> psycopg.AsyncConnection:
        """获取异步数据库连接"""
        return await psycopg.AsyncConnection.connect(self.db_url, row_factory=dict_row)
    ...
    async def get_session_by_url_param(
        self, url_param: str
        ...
        async with await self.get_connection() as conn:
            cursor = conn.cursor()
            await cursor.execute(
                "SELECT * FROM session_mapping WHERE url_param = %s", (url_param,)
            )
    ...
    async def get_session_by_thread_id(
        ...
        async with await self.get_connection() as conn:
            cursor = conn.cursor()
            await cursor.execute(
                "SELECT * FROM session_mapping WHERE thread_id = %s", (thread_id,)
            )
    ...
    async def get_session_by_frontend_uuid(
        self, frontend_uuid: str
    ) -> Optional[SessionMapping]:
        """
        通过前端UUID获取会话
        ...
        async with await self.get_connection() as conn:
            ...
            if row:
                ...
                config.research_config = (
                    row["research_config"]
                    if isinstance(row["research_config"], dict)
                    else (
                        json.loads(row["research_config"])
                        if row["research_config"]
                        else {}
                    )
                )
                config.model_config = (
                    ...
                    if isinstance(row["model_config"], dict)
                        ...
                        json.loads(row["model_config"]) if row["model_config"] else {}
                    )
                )
                config.output_config = (
                    row["output_config"]
                    if isinstance(row["output_config"], dict)
                    else (
                        json.loads(row["output_config"]) if row["output_config"] else {}
                    )
                )
                config.user_preferences = (
                    row["user_preferences"]
                    if isinstance(row["user_preferences"], dict)
                    else (
                        json.loads(row["user_preferences"])
    ...
    async def get_session_overview(self, url_param: str) -> Optional[Dict[str, Any]]:
        ...
        async with await self.get_connection() as conn:
            cursor = conn.cursor()
            await cursor.execute(
                "SELECT * FROM session_overview WHERE url_param = %s", (url_param,)
            )
    ...
    async def get_execution_records_by_session_id(
        self, session_id: int
    ) -> List[Dict[str, Any]]:
        """
        获取会话的执行记录
