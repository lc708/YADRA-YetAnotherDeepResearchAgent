    ...
    try:
        ...
        return AuthResponse(
            user=UserResponse(
                ...
                enable_deep_thinking=user_data.enable_deep_thinking,
                email_confirmed_at=(
                    auth_response.user.email_confirmed_at.isoformat()
                    if auth_response.user.email_confirmed_at
                    else None
                ),
                created_at=(
                    auth_response.user.created_at.isoformat()
                    if hasattr(auth_response.user.created_at, "isoformat")
                    else str(auth_response.user.created_at)
                ),
                updated_at=(
                    (
                        auth_response.user.updated_at or auth_response.user.created_at
                    ).isoformat()
                    if hasattr(
                        auth_response.user.updated_at or auth_response.user.created_at,
                        "isoformat",
                ...
                last_sign_in_at=(
                    auth_response.user.last_sign_in_at.isoformat()
                    if auth_response.user.last_sign_in_at
                    and hasattr(auth_response.user.last_sign_in_at, "isoformat")
                    else (
                        str(auth_response.user.last_sign_in_at)
                        if auth_response.user.last_sign_in_at
                        else None
                    )
                ),
            ...
            session={
                ...
                "expires_in": (
                    auth_response.session.expires_in if auth_response.session else None
                ),
                "token_type": "bearer",
            },
        )
...
async def get_user_info(user_data: Dict[str, Any]) -> UserResponse:
    ...
    return UserResponse(
        ...
        enable_deep_thinking=profile.get("enable_deep_thinking", False)
        or user.user_metadata.get("enable_deep_thinking", False),
        email_confirmed_at=(
        ...
        updated_at=(
            (user.updated_at or user.created_at).isoformat()
            if hasattr(user.updated_at or user.created_at, "isoformat")
            else str(user.updated_at or user.created_at)
        ),
        last_sign_in_at=(
            user.last_sign_in_at.isoformat()
            if user.last_sign_in_at and hasattr(user.last_sign_in_at, "isoformat")
            else str(user.last_sign_in_at) if user.last_sign_in_at else None
        ),
    )
...
async def create_user_profile(
    user_id: str, email: str, display_name: Optional[str], enable_deep_thinking: bool
):
    """创建用户扩展信息"""
    conn = get_db_connection()
    try:
...
async def update_user_profile(
    user_id: str, update_data: UserUpdateRequest
) -> UserResponse:
    """更新用户信息"""
    conn = get_db_connection()
    try:
                        ...
                        user_id,
                        {
                            "user_metadata": {
                                ...
                                "enable_deep_thinking": (
                                    update_data.enable_deep_thinking
                                ),
                            }
                        },
                    )
        ...
        if user_response and user_response.user:
            ...
            return UserResponse(
                ...
                enable_deep_thinking=(
                    result["enable_deep_thinking"] if result else False
                ),
                email_confirmed_at=(
                ...
                last_sign_in_at=(
                    ...
                    and hasattr(user.last_sign_in_at, "isoformat")
                    else str(user.last_sign_in_at) if user.last_sign_in_at else None
                ),
            )
        else:
...
async def create_or_update_task(
    user_id: str, thread_id: str, task_name: Optional[str] = None
) -> TaskResponse:
    """创建或更新用户任务"""
    conn = get_db_connection()
    try:
