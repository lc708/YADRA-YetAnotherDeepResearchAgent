...
class ResumeRequest(BaseModel):
    ...
    checkpoint_id: Optional[str] = Field(
        default=None, description="Specific checkpoint to resume from"
    )
    inputs: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional inputs for resumption"
    )
...
async def get_checkpoints(
    thread_id: str, limit: int = 50, before: Optional[str] = None
) -> List[CheckpointInfo]:
    """
        ...
        async for checkpoint_tuple in checkpointer.alist(config):
            checkpoint_info = CheckpointInfo(
                ...
                parent_checkpoint_id=(
                    checkpoint_tuple.parent_config.get("configurable", {}).get(
                        "checkpoint_id"
                    )
...
async def get_thread_state(
    ...
    try:
        ...
        if not state_snapshot or not state_snapshot.values:
            raise HTTPException(
                status_code=404, detail="Thread or checkpoint not found"
            )
...
async def update_thread_state(
    thread_id: str, update_request: StateUpdate
) -> ThreadState:
    """
    ...
    try:
        ...
        return ThreadState(
            ...
            parent_checkpoint_id=(
                updated_state.parent_config.get("configurable", {}).get("checkpoint_id")
                if updated_state.parent_config
                else None
            ),
        )
    except Exception as e:
...
async def resume_thread_execution(thread_id: str, resume_request: ResumeRequest):
    """
...
async def fork_thread(
    ...
    try:
        ...
        if not source_state:
            raise HTTPException(
                status_code=404, detail="Source thread or checkpoint not found"
            )
            ...
            metadata={
                **new_state.metadata,
                ...
                "forked_from_checkpoint": (
                    checkpoint_id
                    or source_state.config["configurable"]["checkpoint_id"]
                ),
